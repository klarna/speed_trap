...
new_blocked_not_enforced_test() ->
  application:ensure_all_started(speed_trap),
  Id = unique_id(?FUNCTION_NAME),
  ?assertEqual({error, {bad_options, [{blocked, true}, {enforce_rate_limit, false}]}},
               speed_trap:new(Id,
                              #{bucket_size => 10,
                                refill_interval => 1,
                                refill_count => 1,
...
modify_block_test() ->
  ...
  ?assertEqual({ok,
                #{blocked => true,
                  ...
                  enforce_rate_limit => true}},
               speed_trap:options(Id)),
  speed_trap:modify(Id, #{blocked => false}),
  ?assertMatch({ok, _}, speed_trap:try_pass(Id)),
  ?assertEqual({ok,
                #{blocked => false,
                  bucket_size => 10,
...
modify_block_and_not_enforced_test() ->
  application:ensure_all_started(speed_trap),
  Id = unique_id(?FUNCTION_NAME),
  ok =
    speed_trap:new(Id,
  ...
  ?assertEqual({ok,
               ...
               speed_trap:options(Id)),
  ?assertEqual({error, {bad_options, [{blocked, true}, {enforce_rate_limit, false}]}},
               speed_trap:modify(Id, #{enforce_rate_limit => false})),
  ?assertMatch({error, blocked}, speed_trap:try_pass(Id)),
  ?assertEqual({ok,
                #{blocked => true,
                  bucket_size => 10,
                  refill_interval => 100,
                  ...
                  enforce_rate_limit => true}},
               speed_trap:options(Id)),
  ?assertEqual(ok, speed_trap:modify(Id, #{blocked => false, enforce_rate_limit => true})),
  ?assertMatch({ok, _}, speed_trap:try_pass(Id)),
  application:stop(speed_trap).
...
modify_reduce_bucket_test() ->
  ...
  ?assertEqual({error, too_many_requests}, speed_trap:try_pass(Id)),
  ok = speed_trap:modify(Id, #{bucket_size => 2, refill_interval => 1}),
  ?assertEqual({ok,
                #{blocked => false,
                  bucket_size => 2,
                  refill_interval => 1,
...
options_test() ->
  ...
  ?assertEqual({ok, Options#{blocked => false}}, speed_trap:options(Id)),
  application:stop(speed_trap).
...
all_test() ->
  ...
  Options =
    #{bucket_size => 10,
      ...
      delete_when_full => false,
      enforce_rate_limit => true},
  ExpectedValues = Options#{blocked => false, tokens => 10},
  ok = speed_trap:new(Id, Options),
...
bad_options_test() ->
  ...
  BadOpts13 =
    #{bucket_size => 1,
      ...
      refill_count => 1,
      delete_when_full => false,
      enforce_rate_limit => foo},
  ?assertEqual({error, {bad_options, [{enforce_rate_limit, foo}]}},
...
enforce_rate_limit_test() ->
  ...
  ?assertEqual({error, too_many_requests}, speed_trap:try_pass(Id)),
  ok = speed_trap:modify(Id, #{enforce_rate_limit => false}),
